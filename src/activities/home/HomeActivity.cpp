...
int HomeActivity::getMenuItemCount() const {
  int count = 6;  // File Browser, Recents, Morning Briefing, Web Browser, File transfer, Settings
  ...
  auto activateSelection = [this] {
    ...
    switch (indexToMenuItem(menuIndex, hasOpdsServers)) {
      ...
      case HomeMenuItem::RECENTS:
        onRecentsOpen();
        break;
      case HomeMenuItem::MORNING_BRIEFING:
        onMorningBriefingOpen();
        break;
      case HomeMenuItem::WEB_BROWSER:
        onWebBrowserOpen();
        break;
...
void HomeActivity::render(RenderLock&&) {
  ...
  std::vector<const char*> menuItems = {tr(STR_BROWSE_FILES), tr(STR_MENU_RECENT_BOOKS), "The Daily Sticky",
                                        "Web Browser",        tr(STR_FILE_TRANSFER),     tr(STR_SETTINGS_TITLE)};
  std::vector<UIIcon> menuIcons = {Folder, Recent, Bookmark, Text, Transfer, Settings};
...
void HomeActivity::onMorningBriefingOpen() { activityManager.goToMorningBriefing(); }
